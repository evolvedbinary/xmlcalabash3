...
abstract class AbstractInternetProtocolRequest<T, U : InternetProtocolResponse<T>>(open val stepConfig: StepConfiguration, open val uri: URI) : InternetProtocolRequest<T, U> {
    companion object {
     ...
     */
    override var requestTimeout: Int? = null
    /**
     * HTTP Response Timeout in Milliseconds.
     */
    override var responseTimeout: Int? = null
    ...
    override var httpVersion: Pair<Int,Int>? = null
    override var statusOnly = false
    override var suppressCookies = false
    override var overrideContentType: MediaType? = null
    override var followRedirectCount = -1
    override var sendBodyAnyway = false

    override var properties: DocumentProperties
        get() = _properties
        set(value) {
            _properties.setAll(value)
        }

    override var parameters: Map<QName,XdmValue>
        get() = _parameters
        set(value) {
            _parameters.clear()
    ...
    override fun addSource(doc: XProcDocument) {
        _sources.add(doc)
    }
    ...
    override fun authentication(method: String, username: String, password: String, preemptive: Boolean) {
    ...
    override fun execute(method: String): U {
        href = uri
        return executeWithRedirects(method)
    }

    protected abstract fun createResponse(responseUri: URI, statusCode: Int) : U

    private fun executeWithRedirects(method: String): U {
        ...
        val httpResult = try {
            ...
            val response = createResponse(finalURI, 408)
        ...
        val response = createResponse(finalURI, httpResult.code)
        response.cookieStore = cookieStore
    ...
    private fun readResponseEntity(httpResult: HttpResponseEntity, response: U): U {
    ...
    protected open fun readSinglepartEntity(httpResult: HttpResponseEntity, response: U): U {
        ...
        try {
            ...
            var doc = buildDocument(docuri, meta, stream, response.mediaType!!, charset)
            ...
            if (serialization is XdmMap) {
                ...
                if (encoding != null) {
                    ...
                    doc = addDocumentProperties(doc, mapOf(Ns.contentType to mtvalue))
    ...
    protected fun readMultipartEntity(httpResult: HttpResponseEntity, response: U): U {
        ...
        while (reader.readHeaders()) {
            ...
            val doc = buildDocument(baseUri, meta, partStream, mcontentType)
            response.addResponse(doc)
        }
    ...
    protected abstract fun buildDocument(href: URI?, documentProperties: DocumentProperties = DocumentProperties(), stream: InputStream, mediaType: MediaType, charset: Charset? = null) : T

    protected abstract fun addDocumentProperties(responseDocument: T, properties: Map<QName, XdmValue>) : T
    ...
    protected class ResponseEntityHandler(): HttpClientResponseHandler<HttpResponseEntity> {
    ...
    protected class HttpResponseEntity() {
        var code = -1
