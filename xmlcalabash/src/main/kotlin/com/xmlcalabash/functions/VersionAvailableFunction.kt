...
class VersionAvailableFunction(private val config: SaxonConfiguration): ExtensionFunctionDefinition() {
    override fun getFunctionQName(): StructuredQName {
        return StructuredQName("p", NsP.namespace, "version-available")
    }
    ...
    inner class StepAvailableImpl: ExtensionFunctionCall() {
        override fun call(context: XPathContext?, arguments: Array<out Sequence>?): Sequence {
            ...
            return if (version in setOf("3.0", "3.1", "3.2")) BooleanValue.TRUE else BooleanValue.FALSE
        }
    }
}