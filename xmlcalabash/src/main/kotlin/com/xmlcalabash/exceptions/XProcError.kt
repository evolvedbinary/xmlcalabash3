...
open class XProcError protected constructor(val code: QName, val variant: Int, errorLocation: Location, inputLocation: Location, vararg val details: Any) {
    companion object {
        ...
        fun xsMessageAttributeForbidden() = static(Pair(8, 2))
        ...
        fun xsPortNotReadableOnUnknownStep(step: String, port: String) = static(Pair(22, 1), step, port)
        fun xsPortNotReadableOnUnknownStep(step: String) = static(Pair(22, 2), step)
        fun xsPortNotReadableNoSuchPort(step: String, port: String) = static(Pair(22, 3), step, port)
        fun xsPortNotReadable(step: String, port: String) = static(Pair(22, 4), step, port)
        fun xsOutputPortNotReadable(step: String, port: String) = static(Pair(22, 5), step, port)
        ...
        fun xsNoSuchOption(name: QName) = static(Pair(31, 1), name)
        fun xsNoSuchOption(name: QName, stepType: QName) = static(Pair(31, 2), name, stepType)
        fun xsNoSuchOption(name: QName, stepType: QName, stepName: String) = static(Pair(31,3), name, stepType, stepName)
        ...
        fun xsAttributeInPNamespaceNotAllowed(name: QName) = static(97, name)
        ...
        fun xdOutputSequenceForbidden(port: String) = dynamic(Pair(7, 1), port)
        fun xdOutputRequiredOnPort(port: String) = dynamic(Pair(7, 2), port)
        ...
        fun xdBadType() = dynamic(Pair(36,1))
        fun xdBadType(value: String, type: String) = dynamic(Pair(36,2), value, type)
        fun xdBadType(value: String, type: ItemType) = dynamic(Pair(36,3), value, type)
        ...
        fun xcCannotCopy(op: String, source: URI, target: URI) = step(Pair(50,2), op, source, target)
        fun xcCannotCopySameURI(op: String, source: URI, target: URI) = step(Pair(50,2), op, source, target)
        ...
        fun xcXsltRuntimeError(message: String): XProcError {
            val error = step(95, message)
            return error
        }
        ...
        fun xcInvisibleXmlParseFailed() = step(Pair(205, 1))
        fun xcInvisibleXmlParseFailed(failure: XdmNode) = step(Pair(205, 2), failure)
    ...
    var _errorLocation = errorLocation
    val errorLocation: Location
        get() = _errorLocation

    var _inputLocation = inputLocation
    ...
    internal var _reports = mutableListOf<Report>()
    ...
    fun asStatic(): XProcError {
        if (code == NsErr.xd(36)) {
            ...
            error.updateAt(errorLocation)
            error.updateReports(reports)
    ...
    fun at(saxonLocation: net.sf.saxon.s9api.Location?): XProcError {
        if (saxonLocation != null && saxonLocation.systemId != null) {
            ...
            val error = XProcError(code, variant, errorLocation, xloc, *details)
            error._moreDetails.addAll(moreDetails)
            error._reports.addAll(reports)
            return error
        }
    ...
    fun at(node: XdmNode): XProcError {
        ...
        val error = XProcError(this, errorLocation, Location(locNode))
        error._moreDetails.addAll(moreDetails)
        error._reports.addAll(reports)
        return error
    }
    ...
    fun at(doc: XProcDocument): XProcError {
        val error = XProcError(this, errorLocation, Location(doc.baseURI))
        error._throwable = throwable
        error._moreDetails.addAll(moreDetails)
        error._reports.addAll(reports)
        return error
    ...
    fun updateAtInput(location: Location): XProcError {
        if (location != Location.NULL) {
    ...
    fun updateReports(reports: List<Report>): XProcError {
        _reports.clear()
    ...
    fun with(newCode: QName): XProcError {
        val error = XProcError(newCode, 1, errorLocation, inputLocation, *details)
        error._throwable = throwable
        error._moreDetails.addAll(moreDetails)
        return error
    }
