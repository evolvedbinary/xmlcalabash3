...
val xincludeMarkuptesting = tasks.register<SaxonXsltTask>("xincludeMarkuptesting") {
  dependsOn(rngArchiveManifestSchema)
  dependsOn(rngPipelineMessagesSchema)
  dependsOn(rngFilesetSchema)
  ...
  inputs.file(rngArchiveManifestSchema.get().outputs.getFiles().getSingleFile())
  inputs.dir(layout.buildDirectory.dir("examples"))
  inputs.dir(layout.projectDirectory.dir("src/markuptesting"))
  inputs.dir(layout.projectDirectory.dir("src/examples"))
  inputs.dir(layout.projectDirectory.dir("../xmlcalabash/src/main/resources/com/xmlcalabash"))
  input(layout.projectDirectory.file("src/markuptesting/testing.xml"))
  stylesheet(layout.projectDirectory.file("src/xsl/xinclude.xsl"))
  output(layout.buildDirectory.file("mtbuild/markuptesting.xml").get())
  args(listOf("-init:org.docbook.xsltng.extensions.Register"))
  parameters (
      mapOf(
          "docbook-transclusion" to "true"
...
val processMarkuptesting = tasks.register<JavaExec>("processMarkuptesting") {
  dependsOn("translateDocBook")
  dependsOn("xincludeMarkuptesting")

  inputs.file(xincludeMarkuptesting.get().outputs.getFiles().getSingleFile())
  inputs.file(layout.projectDirectory.file("src/xpl/process.xpl"))
  outputs.file(layout.buildDirectory.file("mtbuild/processed.xml"))

  classpath = configurations.named("documentation").get()
  mainClass = "com.xmlcalabash.app.Main"
  args(layout.projectDirectory.file("src/xpl/process.xpl"),
       "-i:source=${xincludeMarkuptesting.get().outputs.getFiles().getSingleFile()}",
       "-o:result=${layout.buildDirectory.file("mtbuild/processed.xml").get()}")
}

val validateMarkuptesting = tasks.register<RelaxNGValidateTask>("validateMarkuptesting") {
  dependsOn("translateDocBook")
  dependsOn("processMarkuptesting")

  inputs.file(docbookRNG.get().outputs.getFiles().getSingleFile())
  inputs.file(processMarkuptesting.get().outputs.getFiles().getSingleFile())
  outputs.file(layout.buildDirectory.file("mtbuild/validated.xml"))

  input(processMarkuptesting.get().outputs.getFiles().getSingleFile())
  schema(docbookRNG.get().outputs.getFiles().getSingleFile())
  output(layout.buildDirectory.file("mtbuild/validated.xml").get())
}

val markupTesting = tasks.register<SaxonXsltTask>("markupTesting") {
  dependsOn("copyMarkuptestingResources")
  dependsOn("validateMarkuptesting")
  dependsOn("xmlCalabashVersion")
  dependsOn("xmlCalabashBuildInfo")
  ...
  inputs.dir(layout.projectDirectory.dir("src/xsl"))
  inputs.file(layout.buildDirectory.file("archive-manifest.rng"))
  outputs.files(fileTree("dir" to layout.buildDirectory.dir("markuptesting/current"),
                         "include" to "*.html"))

  input(validateMarkuptesting.get().outputs.getFiles().getSingleFile())
  stylesheet(layout.projectDirectory.file("src/xsl/reference.xsl"))
  output(layout.buildDirectory.file("markuptesting/current/index.html").get())
  args(listOf("-init:org.docbook.xsltng.extensions.Register"))
  parameters (
      mapOf(
          "mediaobject-input-base-uri" to "file:${layout.buildDirectory.get()}/markuptesting/current/",
          "chunk-output-base-uri" to "${layout.buildDirectory.get()}/markuptesting/current/",
          "dep_fop" to project.findProperty("fop").toString()
      )
  )

  doLast {
    val stream = PrintStream(layout.buildDirectory.file("markuptesting/details.json").get().asFile)
    stream.println("{\"version\": \"${refVersion}\", \"pubdate\": \"${xmlbuild.buildTime.get()}\"}")
    stream.close()
  }
}

tasks.register("copyMarkuptestingJarResources") {
  outputs.dir(project.layout.buildDirectory.dir("markuptesting/current"))
  ...
  doLast {
    ...
    copy {
      from(zipTree(dbjar.toString()))
      into(layout.buildDirectory.dir("markuptesting/current"))
      include("org/docbook/xsltng/resources/**")
      eachFile {
        relativePath = RelativePath(true, *relativePath.segments.drop(4).toTypedArray())
      }
  ...
  doLast {
    delete(project.layout.buildDirectory.dir("markuptesting/org"))
  }
}

tasks.register<Copy>("copyMarkuptestingStaticResources") {
  into(layout.buildDirectory.dir("markuptesting/current"))
  from(layout.projectDirectory.dir("src/resources")) {
    exclude("img/*.psd")
  }
...
tasks.register("copyMarkuptestingResources") {
  dependsOn("copyMarkuptestingJarResources", "copyMarkuptestingStaticResources")
}
